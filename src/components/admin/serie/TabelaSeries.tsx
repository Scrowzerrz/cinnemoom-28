...
import { EditarSerieDialog } from './EditarSerieDialog';
...
export function TabelaSeries({ series, carregando, excluindo, onDelete, onEditSuccess }: TabelaSeriesProps) {
  const [serieEditando, setSerieEditando] = useState<string | null>(null);
  
  const abrirEdicao = (serieId: string) => {
    setSerieEditando(serieId);
  };
  ...
  return (
    <>
      ...
      <Dialog open={!!serieEditando} onOpenChange={(open) => !open && setSerieEditando(null)}>
        {serieEditando && (
          <EditarSerieDialog 
            serieId={serieEditando} 
